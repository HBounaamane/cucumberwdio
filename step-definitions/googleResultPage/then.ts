...
Then(
  /^links related to (.*) are shown on the result page$/,
  async (keyword) => {
...
Then(/no links related to (.*)are shown/, async (keyword) => {
  ...
  const resultMessage =
    await googleResultPage.getTextResultOfGoogleHeadMessage();
  console.log(`Your search - ${keyword} - did not match any documents`);
  assert(isResultDisplayed === false, "Results are displayed !!!");

  assert(
    resultMessage.includes(`did not match any documents`),
    "links are displayed !!"
  );
});

Then(
  /a List box is displayed showing suggestions related to user search/,
  async () => {
    const isSuggestionDisplayed = await googleResultPage.isListBoxDisplayed();

    assert(
      isSuggestionDisplayed === true,
      "no suggestion for your research !!!"
    );
  }
);
