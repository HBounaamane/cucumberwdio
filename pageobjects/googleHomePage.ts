...
class GoogleHomePage extends Page {
  ...
  public async acceptGoogleCookies() {
    try {
      const isCookiesModalDisplayed = await this.cookiesModal.waitForDisplayed({
        timeout: 4000,
      });
  ...
  public async waitForGoogleSearchPageToBeReady() {
    ...
    await googleIconHomePage.waitForDisplayed({
      timeout: 5000,
      timeoutMsg: "google home page is not ready",
    });
  }

  public async googleSearch(keyword: string, clear: boolean = false) {
    const googleSearchField = await this.searchField;
    await googleSearchField.waitForDisplayed({
      timeout: 3000,
      timeoutMsg: "Google search field is not displayed",
    });
    if (clear) {
